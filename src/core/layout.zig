...
pub fn build(arena: std.mem.Allocator, doc: *Document, vp: *const Viewport) error{OutOfMemory}!Layout {
...
test "Layout: first line clipped horizontally" {
    var a = std.heap.ArenaAllocator.init(std.testing.allocator); defer a.deinit();
    ...
    const layout = try build(alloc, &doc, &vp);
